...
def origin_destination_connection_matrix(df, time_filter, frequency):
    ...
    prep = df.where(time_filter).withColumn('call_datetime_lag', F.lag('call_datetime')\
        ...
        .where(F.col('od_count') > privacy_filter)

    result = result.join(prep, (prep.region == result.region_to) & (prep.region_lag == result.region_from) & (prep.day == result.connection_date), 'full')\
